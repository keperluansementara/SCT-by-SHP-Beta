 ...
 * rankingEngine.js
 * ─────────────────────────────────────────────────────────────────────────────
 * Smart Alert Prioritization Engine.
 * Scores business issues across all domains on a 0–100 scale and ranks them
 * by severity. Produces a prioritized issue list and alert banner sentence.
 *
 * Source: AlertEngine (SCT-by-SHP.html lines 3076–3445)
 *
 * Dependencies:
 *   business/timegoneEngine.js — TimeEngine.get(), TimeEngine.pace(),
 *                                TimeEngine.fmt(), TrendEngine.insight()
 *   state.js                   — State.timeEngine (valid, source)
 *   utils/helpers.js           — Utils.fmtPct(), Utils.fmtCompact()
 *
 * ── Scoring model ────────────────────────────────────────────────────────────
 *   Dimension A (0–40 pts) — achievement vs pace gap
 *   Dimension B (0–25 pts) — negative vs-LM growth
 *   Dimension C (0–10 pts) — negative vs-LY growth (secondary signal)
 *   Dimension D (0–15 pts) — zero-transaction ratio among outlets
 *   Dimension E (0–10 pts) — required RR vs actual RR multiplier
 *   Total capped at 100 pts per issue.
 *
 * ── Badge thresholds ─────────────────────────────────────────────────────────
 *   70+ → 🔴 CRITICAL
 *   45+ → 🔴 DANGER
 ...
 *   - No KPI calculations (reads from k = State.kpi passed as parameter)
 *   - No DOM rendering
 *   - No State writes
 *
 * Public API:
 *   AlertEngine.generate(k)               → { issues[], top5[], topIssue }
 *   AlertEngine.bannerSentence(alerts, k) → string
 * ─────────────────────────────────────────────────────────────────────────────
 */
